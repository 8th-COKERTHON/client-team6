...
| 전체 랭킹 | `GET` | `/api/v1/rankings` | 현재 회원 에피소드만 필터링해 연동 |
...
- 인증된 `/episodes`를 cursor 끝까지 조회해 현재 회원의 에피소드 ID 집합을 만듭니다.
- `/rankings?page&size`를 필요한 페이지까지 조회한 뒤 해당 ID 집합과 교차하므로 다른 회원의 기록은 화면 데이터에서 제외됩니다.
- 표시 순위는 필터링된 현재 회원 에피소드를 점수순으로 정렬해 1위부터 다시 계산합니다.
- 랭킹 API에 검색 조건이 없으므로 `/episodes/search` 결과를 위의 개인 랭킹과 다시 교차합니다.
- 챔피언 기록도 현재 회원의 에피소드 ID에 포함된 항목만 사용합니다.
